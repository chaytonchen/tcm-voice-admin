  ...
  <view>
    <view class="uni-header">
      ...
      <view class="uni-group">
        ...
        <button v-if="hasPermission('CREATE_UNI_CMS_ARTICLE')" class="uni-button" type="primary" size="mini"
          @click="navigateTo('./add')">新增</button>
        <button v-if="hasPermission('DELETE_UNI_CMS_ARTICLE')" class="uni-button" type="warn" size="mini"
    ...
    <view class="uni-container">
      <unicloud-db ref="udb" :collection="collectionList" :where="where" page-data="replace" :orderby="orderby"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection"
          ...
          <uni-tr>
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'thumbnail')" sortable
              @sort-change="sortChange($event, 'thumbnail')">封面大图
            </uni-th>
            ...
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'excerpt')" sortable
              @sort-change="sortChange($event, 'excerpt')">摘要
            </uni-th>
            ...
            <uni-th align="center" filter-type="select" :filter-data="options.filterData.categories"
              @filter-change="filterChange($event, 'category_id._id')">分类</uni-th>
            <uni-th align="center" filter-type="select" :filter-data="options.filterData.article_status_localdata"
            ...
            <uni-th align="center" filter-type="range" @filter-change="filterChange($event, 'view_count')" sortable
              @sort-change="sortChange($event, 'view_count')">阅读数量
            </uni-th>
          ...
          <uni-tr v-for="(item, index) in tableList" :key="index">
            <uni-td align="center" v-if="item.thumbnail && item.thumbnail.length > 0">
              <image :src="item.thumbnail[0]" mode="aspectFill"
                ...
                @click="previewCover(item.thumbnail)">
              </image>
            ...
            <uni-td align="center">
              <view class="uni-group">
                <button v-if="hasPermission('UPDATE_UNI_CMS_ARTICLE')"
                  @click="navigateTo('./edit?id=' + item._id, false)" class="uni-button" size="mini" type="primary">修改
                </button>
                <button v-if="item.article_status === 1 && hasPermission('UPDATE_UNI_CMS_ARTICLE')"
                  @click="takeOffArticle(item._id)" class="uni-button" size="mini" type="default">下架</button>
                <button v-if="hasPermission('DELETE_UNI_CMS_ARTICLE')" @click="confirmDelete(item._id)"
