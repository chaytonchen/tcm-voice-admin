...
<script setup>
  ...
  const handleBatchDelete = async () => {
    if (tableRef.value?.selectedIds?.length > 0) {
      ...
      if (confirmResult) {
        ...
        const result = await remove('tcm-shops', where)
        if (result.success) {
          msg('删除成功', 'success')
          fetchData()
        } else {
  ...
  const handleDelete = async (row) => {
    ...
    if (confirmResult) {
      ...
      const result = await remove('tcm-shops', row._id)
      if (result.success) {
        msg('删除成功', 'success')
        fetchData()
      } else {
  ...
  const fetchData = async () => {
    ...
    if (searchForm.keyword) {
      ...
      conditions.push(
        `${new RegExp(keyword)}.test(name) || ${new RegExp(keyword)}.test(contact_person)`
      )
    }
    ...
    const options = {
      ...
      // 所有用到的字段，以字符串形式传入
      field: '_id, name, contact_person, contact_phone, status, created_at'
    }

    // 获取数据 - 正确的调用方式，传递3个参数
    const res = await find('tcm-shops', where, options)
